...
interface AppContextType {
  ...
  sendReplyMessage: (threadId: string, text: string) => void;
...
export const AppProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const [recruiterJobs, setRecruiterJobs] = useState<RecruiterJobPosting[]>(() => {
    try {
      const saved = localStorage.getItem(STORAGE_KEY_RECRUITER_JOBS);
      if (saved) return JSON.parse(saved);
    } catch {
      // fallback
    }
    return INITIAL_RECRUITER_JOBS;
  });
  ...
  const addPlacementDrive = (
    ...
    const eligibleStudents = institutionalStudents.filter(
      (st) => (driveData.minCgpa ? st.cgpa >= driveData.minCgpa : true)
    );
    const initialRegIds =
      driveData.registeredStudentIds ||
      (eligibleStudents.length > 0
        ? eligibleStudents.map((s) => s.id)
        : institutionalStudents.slice(0, 5).map((s) => s.id));

    const newDrive: PlacementDrive = {
      ...driveData,
      id: `drv_${Date.now()}`,
      registeredStudentsCount: driveData.registeredStudentsCount ?? initialRegIds.length,
      shortlistedCount: driveData.shortlistedCount ?? (driveData.shortlistedStudentIds?.length || 0),
      selectedCount: driveData.selectedCount ?? 0,
      registeredStudentIds: initialRegIds,
      shortlistedStudentIds: driveData.shortlistedStudentIds || [],
    };
  ...
  useEffect(() => {
    try {
      localStorage.setItem(STORAGE_KEY_RECRUITER_JOBS, JSON.stringify(recruiterJobs));
    } catch (e) {
      console.warn('Storage save failed', e);
    }
  }, [recruiterJobs]);
  ...
  const updateJobStatus = (jobId: string, status: RecruiterJobPosting['status']) => {
    setRecruiterJobs((prev) => prev.map((j) => (j.id === jobId ? { ...j, status } : j)));
  };
  ...
  const opportunities = useMemo(() => {
    return INITIAL_OPPORTUNITIES.map((opp) => {
      const matchScore = calculateOpportunityMatch(profile.skills, opp);
      return {
        ...opp,
        matchScore,
      };
      ...
      timeline: [
        {
          ...
          note: notes || 'Submitted application via AcademiaConnect Student Portal',
        },
      ],
      notes,
    };
  ...
  const sendReplyMessage = (threadId: string, text: string) => {
    if (!text.trim()) return;
    setMessages((prev) =>
      prev.map((th) => {
        if (th.id === threadId) {
