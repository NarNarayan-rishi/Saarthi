...
export const MessagesPage: React.FC = () => {
  const { messages, sendReplyMessage, activeMessageThreadId, setActiveMessageThreadId } = useApp() as any;
  const [activeThreadId, setActiveThreadId] = useState<string>(activeMessageThreadId || messages[0]?.id || '');

  // Keep local state synced with context
  React.useEffect(() => {
    if (activeMessageThreadId) {
      setActiveThreadId(activeMessageThreadId);
    }
