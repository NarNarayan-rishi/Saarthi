...
export const ApplicationCard: React.FC<ApplicationCardProps> = ({
  ...
  const { messages, setActiveMessageThreadId, withdrawApplication, setActiveTab } = useApp() as any;
  const relatedThread = messages?.find((m: any) => m.senderName === application.company);
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        ...
        <div>
          <div className="flex items-center gap-2 flex-wrap mb-1.5">
            ...
            {application.source && application.source !== 'Internal' && (
              <span className={`text-[10px] font-bold px-2.5 py-0.5 rounded-full border ${
                application.source === 'LinkedIn' ? 'bg-blue-50 text-blue-800 border-blue-200' :
                application.source === 'Naukri' ? 'bg-emerald-50 text-emerald-800 border-emerald-200' :
                application.source === 'Internshala' ? 'bg-sky-50 text-sky-800 border-sky-200' :
        ...
        <div className="flex items-center gap-2 self-end sm:self-center">
          {application.externalUrl && (
            <a
              ...
              className="px-3.5 py-2 bg-indigo-50 hover:bg-indigo-100 text-indigo-700 rounded-xl text-xs font-bold flex items-center gap-1 border border-indigo-200 transition-colors"
              title={`View original post on ${application.source}`}
            >
              <span>Overview</span>
          ...
          {relatedThread && (
            <button
              onClick={() => {
                setActiveMessageThreadId(relatedThread.id);
                setActiveTab('messages');
              }}
              className="px-3.5 py-2 bg-indigo-50 text-indigo-700 hover:bg-indigo-100 rounded-xl font-bold text-xs flex items-center justify-center gap-2 transition-colors mr-2 border border-indigo-100"
            >
