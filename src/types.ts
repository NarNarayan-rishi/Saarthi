...
export interface Opportunity {
  ...
  postedBy?: 'Recruiter' | 'Institution' | 'System';
  matchScore?: number; // dynamically computed based on student skills
  source?: 'Internal' | 'LinkedIn' | 'Naukri' | 'Internshala';
  externalUrl?: string;
}
...
export type PipelineStageType = 'Interview' | 'Live Problem Solving' | 'Specific Task' | 'PDF to CBT Test';
...
export interface PipelineStage {
  ...
  title: string;
  config?: any; // e.g. meetLink, timeLimit, taskDescription, uploadedFileName
}
...
export interface JobApplicant {
  ...
  jobId: string;
  candidateId: string;
  candidate: RecruiterCandidate; // The embedded candidate data
  appliedDate: string;
  status: 'Applied' | 'Under Process' | 'Hired' | 'Rejected';
