          ...
          <div className="space-y-4 pt-2">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {filteredArticles.map((article) => (
                <div
                  key={article.id}
                  className="p-4 rounded-2xl bg-white border border-slate-200/90 hover:border-indigo-300 hover:shadow-xs transition-all flex flex-col justify-between space-y-3 group"
                >
                  <div className="space-y-2">
                    ...
                    {/* Title */}
                    <h5 className="font-['Outfit'] font-bold text-sm text-slate-900 group-hover:text-indigo-600 transition-colors line-clamp-2">
                      {article.title}
                    </h5>
                    ...
                    {/* Student Actionable Impact Box */}
                    {article.impact && (
                      <div className="p-2.5 rounded-xl bg-amber-50/70 border border-amber-200/80 text-[11px] text-amber-900 leading-snug">
                        <span className="font-bold text-amber-950 block mb-0.5">🎯 Student Takeaway:</span>
                        {article.impact}
                      </div>
                    )}
                  </div>

                  {/* Footer: Tags & External Source Link */}
            ...
            {filteredArticles.length === 0 && (
              <div className="p-8 text-center bg-slate-50 rounded-2xl border border-slate-200 text-xs text-slate-500">
                No news articles match category "{selectedCategory}". Try selecting "All".
              </div>
            )}
          </div>
